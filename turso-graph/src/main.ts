...
function move(x1: number, y1: number, x2: number, y2: number, duration: number): [number, number] {
...
async function render(duration: number) {
  if (db == null) { return; }
  rendering = true;
...
async function init() {
  ...
  addEventListener("click", async (event) => {
    if (pendingStartPoint != null) { return; }
    let mouseX = event.pageX - canvas.getBoundingClientRect().x;
    let mouseY = event.pageY - canvas.getBoundingClientRect().y;
